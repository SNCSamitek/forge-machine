...
int Arm::setJointAngles(int x, int y) {
  ...
  if((ll1 + ll2) < distanceToTarget || abs(ll1 - ll2) > distanceToTarget){
    Serial.println("the crane arm cannot get to that target");
    return 0;
  }

  double theta2 = acos((x*x + y*y - ll1*ll1 - ll2*ll2 / 2*(ll1 * ll2)));
  double theta1 = atan2(y, x) - atan2((ll2*sin(ll2)), (ll1 + (ll2*cos(ll2))));
